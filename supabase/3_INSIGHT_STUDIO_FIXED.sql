...
-- Drop existing harvester tables if they exist (clean slate)
DROP TABLE IF EXISTS web_scraping_jobs CASCADE;
DROP TABLE IF EXISTS manufacturer_monitoring CASCADE;
...
CREATE INDEX idx_youtube_videos_manufacturer ON youtube_videos(manufacturer);
CREATE INDEX idx_youtube_videos_channel_name ON youtube_videos(channel_name);
...
CREATE INDEX idx_competitor_pricing_manufacturer ON competitor_pricing(manufacturer);
CREATE INDEX idx_competitor_pricing_model ON competitor_pricing(model);
CREATE INDEX idx_competitor_pricing_source ON competitor_pricing(source);

-- Manufacturer Monitoring
CREATE TABLE manufacturer_monitoring (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_manufacturer_monitoring_enabled ON manufacturer_monitoring(enabled);
...
CREATE INDEX idx_web_scraping_jobs_job_type ON web_scraping_jobs(job_type);
